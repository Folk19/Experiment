...
namespace IOB2_format_evaluate
{
    class Program
    {
        ...
        {
            StreamReader sr = new StreamReader(@".\output.data"/*args[0]*/);

            List<string> harmonicMeanType = new List<string>();
            for (int i = 1; i < args.Length; i++)
                harmonicMeanType.Add(args[i]);

            Dictionary<Tuple<int, int>, string> answerList = new Dictionary<Tuple<int, int>, string>(); //tuple<beginPosition, endPosition>
            ...
            for (int loop = 1; !sr.EndOfStream; loop++ )
            {
                ...
                else if (line.StartsWith("#") && tabCount.Count() == 0)
                {
                    continue;
                }
                ...
                //ans
                if ((!nowStatAns.Equals(answer) || tokens[tokens.Length - 2].StartsWith("B")) && !nowStatAns.Equals(String.Empty))
                {
                    answerList.Add(new Tuple<int, int>(beginPositionAns, loop - 1), nowStatAns);
                    answerList.Add(new Tuple<int, int>(beginPositionAns, -1), nowStatAns);
                    answerList.Add(new Tuple<int, int>(-1, loop - 1), nowStatAns);
                ...
                //predict
                if ((!nowStatPre.Equals(predict) || tokens[tokens.Length - 1].StartsWith("B")) && !nowStatPre.Equals(String.Empty))
                    predictList.Add(new Tuple<int, int>(beginPositionPre, loop - 1), nowStatPre);
                if (!nowStatPre.Equals(predict) || tokens[tokens.Length - 1].StartsWith("B") || nowStatPre.Equals(String.Empty))
        ...
        {
            return IOB2.Contains('-') ? IOB2.Substring(IOB2.IndexOf('-') + 1) : IOB2 != "O" ? "A" : IOB2;
        }
    }
}
